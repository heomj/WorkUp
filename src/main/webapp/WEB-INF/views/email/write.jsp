...
<div id="mail-success-view" style="display: none;">
    <div class="mb-4 d-flex align-items-center justify-content-between">
        <div style="color: #2c3e50; display: flex; align-items: center; gap: 10px;">
            ...
            <div style="display: flex; align-items: baseline; gap: 8px;">
                ...
                <span style="font-weight: normal; color: #717171; font-size: 15px;">| 메일 쓰기</span>
            </div>
        </div>
    </div>
    ...
    <div class="dashboard-card p-5 text-center">
        ...
        <div class="d-flex justify-content-center gap-3">
            <a class="btn btn-primary d-inline-flex align-items-center px-4" href="/email">
                <span class="material-icons fs-6 me-1">list</span>
                목록으로 이동
            </a>
            <a class="btn btn-outline-secondary d-inline-flex align-items-center px-4"href="/email/write">
...
<div id="mail-write-view" >
    <div class="mb-4 d-flex align-items-center justify-content-between">
        ...
        <button class="btn btn-outline-secondary btn-sm" onclick="location.href='/email'">취소</button>
    </div>

    <div class="dashboard-card p-4">
        <ul class="nav nav-tabs" id="mailWriteTabs" role="tablist">
            <li class="nav-item">
                <button class="nav-link active" id="internal-mail-tab" data-bs-toggle="tab" data-bs-target="#internal" type="button">내부 메일</button>
            </li>
            <li class="nav-item">
                <button class="nav-link " id="external-mail-tab" data-bs-toggle="tab" data-bs-target="#external" type="button">외부 메일</button>
            </li>
            <li class="nav-item">
                <button class="nav-link" id="dummy" type="button">더미 데이터 채우기</button>
            </li>
        </ul>
        <div class="tab-content pt-3" id="mailWriteTabContent">
            <div class="tab-pane fade show active" id="internal">
                <form action="/email/send" method="post" enctype="multipart/form-data" id="sendMailForm">
                    ...
                    <div class="mb-3"><label class="form-label fw-bold" >제목&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        ...
                        <input type="text" class="form-control" id="write-subject" maxlength="300" name="emlTtl"/>
                        <!-- value="[보고] 프로젝트 '알파' 진행 현황 및 주간 리포트 (2월 4주차)"-->
                        </div>
                    ...
                    <div class="mb-3"style="margin-bottom: 0 !important;">
                        ...
                        <span class="form-text" style="padding-left: 20px;">여러 파일을 선택하려면 Ctrl 키를 누른 채 클릭하세요.</span>
                        <input type="file" class="form-control mb-3" id="multipartFiles" multiple name="multipartFiles" >
                        <div id="newFileList" class="d-flex flex-wrap"></div>
                        <div class="attachment-section">
                            <c:if test="${not empty emailVO.fileTbVO }">
                            ...
                            <div class="mb-4" id="oldFileList">
                                ...
                                <div class="d-flex flex-wrap">
                                    <c:forEach var="fileDtl" items="${emailVO.fileTbVO.fileDetailVOList}">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="form-control" id="contentsTemp" style="max-height: 500px; overflow: auto;"></div>
                    </div>
                    <div class="mb-3">
                        <div class="text-end"><button class="btn btn-primary px-4" type="button" style="background-color:var(--point-color); border-color: var(--point-color);" onclick="sendMail()">보내기</button></div></div></form>
            </div>

            <div class="tab-pane fade" id="external">
                <form action="/email/sendExternal" method="post" enctype="multipart/form-data" id="extsendMailForm">

                    <div class="mb-3"><label class="form-label fw-bold">받는 사람 </label>
                        <input class="form-control" id="emlOtsdRcvrAddr" name="emlOtsdRcvrAddr"
                               placeholder="외부 메일 주소를 입력하십시오. 예: aaa@gmail.com"/></div>

                    <div class="mb-3"><label class="form-label fw-bold" >제목&nbsp;&nbsp;&nbsp;&nbsp;</label>
                        <input type="text" class="form-control" id="extwrite-subject" name="emlTtl"
                        ></div>
                    <div class="mb-3"style="margin-bottom: 0 !important;">
                        <label class="form-label fw-bold" for="multipartFiles" >첨부파일</label>
                        <span class="form-text" style="padding-left: 20px;">여러 파일을 선택하려면 Ctrl 키를 누른 채 클릭하세요.</span>
                        <input type="file" class="form-control mb-3" id="extmultipartFiles" multiple name="extmultipartFiles" >
                        <div id="newFileList" class="d-flex flex-wrap"></div>
                        <div class="attachment-section">
                            <c:if test="${not empty emailVO.fileTbVO }">
                                <p class="small text-muted mb-3 fw-bold"><i class="material-icons align-middle" style="font-size:16px">add_circle_outline</i> 신규 파일 추가</p>
                                <div class="mb-4" id="extoldFileList">
                                    <p class="small text-muted mb-3 fw-bold"><i class="material-icons align-middle" style="font-size:16px">history</i> 기존 업로드된 파일</p>
                                    <div class="d-flex flex-wrap">
                                        <c:forEach var="fileDtl" items="${emailVO.fileTbVO.fileDetailVOList}">
                                        <span class="file-tag exttag-old" id="extoldFile_${fileDtl.fileDtlId}">
                                            <i class="material-icons me-1" style="font-size:16px">description</i>
                                            ${fileDtl.fileDtlONm}
                                            <span class="material-icons del-icon" onclick="fn_deleteOldFile('${fileDtl.fileDtlId}')">cancel</span>
                                        </span>
                    ...
                    <div class="mb-3">
                        <label class="form-label fw-bold">내용</label>
                        <div class="form-control" id="extcontentsTemp"></div>
                    </div>
                    <div class="mb-3">
                        <div class="text-end"><button class="btn btn-primary px-4" type="button" onclick="sendExternalMail()">보내기</button></div></div></form>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    document.addEventListener("DOMContentLoaded", function () {
        ...
        let recipients = [];
        <%
            List<EmployeeVO> list = (List<EmployeeVO>) request.getAttribute("recipients");
            if (list != null) {
        ...
        %>
        let isEmailVO="${emailVO}";
        let replyTemplate=""
//////////////////////////////////////////////////////////////////////////////답장의 경우/////////////////////////////////
        if(isEmailVO) {
            const emlSndrId = '${emailVO.emlSndrId}';
            const empNm = '${emailVO.empNm}';
            const emlTtl = '${emailVO.emlTtl}';
            const emlSndngDt = '${emailVO.emlSndngDt}';
            /*html += '<div class="text-end text-muted small pe-2" style="width:200px;">' <<<<<날짜 포멧팅 할 것
                + item.emlSndngDt.substring(0, 10)
                + '&nbsp;&nbsp;&nbsp;' + item.emlSndngDt.substring(11, 16) + '</div>';*/
            const emlCn = '${emailVO.emlCn}';
            console.log("발신자(emlSndrId):", emlSndrId);
            console.log("제목(emlTtl):", emlTtl);
            console.log("날짜(emlDt):", emlSndngDt);
            console.log("내용(emlCn):", emlCn);
                // 1. 수신자(멘션) 태그 생성


            console.log("emailVO.replyorforward", '${emailVO.replyorforward}');
                // 2. 제목 세팅 (Re: 붙이기)
            let receiverHtml='';
                            if('reply'=='${emailVO.replyorforward}'){
                                console.log("답장 칸");
                                receiverHtml = `
                    <span class="mention-btn" data-no="\${emlSndrId}" contentEditable="false">
                        @\${emlSndrId} <a class="del-mention">X</a>
                    </span>`;
                                document.getElementById("write-subject").value = "Re: " + emlTtl;
                                document.getElementById("extwrite-subject").value = "Re: " + emlTtl;
                                document.getElementById("receiver").innerHTML = receiverHtml;
                            }else {
                                console.log("전달 칸");
                                document.getElementById("write-subject").value = "(전달)" + emlTtl;
                                document.getElementById("extwrite-subject").value = "(전달)" + emlTtl;
                                document.getElementById("receiver").tabindex="0";
                ...
                // 3. 에디터 본문 조합
                    ...
                    <br><br>
                    <p>---------- 원본 메일 ----------</p>
                    <p><b>보낸 사람:</b> \${emlSndrId}</p>
                    <p><b>날짜:</b> \${emlSndngDt}</p>
                    <p><b>제목:</b> \${emlTtl}</p>
                    <br>
                `+emlCn;
...
// CKEditor
        ClassicEditor
            ...
            .then(editor => {
                ...
                if(isEmailVO) {
                    window.editor.setData(replyTemplate);
                }
            })
            .catch(error => {
        ...
        recipients.forEach(recipient=>{
            console.log('현재 반복 중인 객체:', recipient);
            console.log('empId 값:', recipient.empId);
        document.getElementById("receiver").innerHTML+=`
        <span class="mention-btn" data-no="`+recipient.empId+`">@`+recipient.empNm+`<a
            class="del-mention">X</a></span>`;})

        //외부메일 내용칸
        ClassicEditor
            .create(document.querySelector('#extcontentsTemp'), {
                // 툴바 구성 (필요한 기능만 넣으세요)
                toolbar: [
                    'heading', '|', 'bold', 'italic', 'link', 'bulletedList',
                    'numberedList', '|', 'outdent', 'indent', '|',
            ...
            .then(editor => {
                ...
                window.editorext = editor;
                console.log('에디터가 성공적으로 로드되었습니다.');
                if(isEmailVO) {
                    window.editorext.setData(replyTemplate);
                }
            })
            .catch(error => {
                console.error('에디터 로드 중 오류 발생:', error);
    ...
    function toggleEmrg() {
        ...
        if (input.value === 'N') {
            ...
            btn.innerText = '긴급 X';
            // 흰 바탕 + 빨간 글씨로 변경
            btn.class = 'btn btn-outline-danger btn-xs';
        }
    }
    //메일 발송
    const sendMail = async ()=>{
        ...
        if(mentionSpans.length === 0){
            AppAlert.error("받는 사람을 한 명 이상 설정해야 합니다.",'',null,'edit_note');
        ...
        // 화면에 떠 있는 기존 파일 태그(.tag-old)들을 다 찾아옴
        const oldFileSpans = document.querySelectorAll('.tag-old');
        oldFileSpans.forEach(span => {
            // id가 "oldFile_123" 이면 "123"만 추출해서 'keepFileIds'라는 이름으로 담아
            const fileDtlId = span.id.replace('oldFile_', '');
            formData.append('keepFileIds', fileDtlId);
        });

        const subject = formData.get('emlTtl'); // input의 name="emlTtl"

        if (!subject || subject.trim() === "") {
            AppAlert.error("제목을 입력해주세요.",'',null,'title');
            document.getElementById('write-subject').focus();
            return false; // 전송 중단
        }

// 2. 글자 수 검사 (300자)
        if (subject.length > 300) {
            alert("제목은 300자 이내로 입력해야 합니다.");
            return false;
        }
    ...
    //외부메일
    async function sendExternalMail(){
        const extformElements = document.getElementById('extsendMailForm');
        const extformData = new FormData(extformElements);
        extformData.append('emlCn', window.editorext.getData()); //내용

        axios.post("/email/sendext",extformData).then(response=>{
            document.getElementById('mail-write-view').style.display = 'none';//메일쓰기
            document.getElementById('mail-success-view').style.display = 'block';
            // --- [ 폼 초기화 시작 ] ---
            clearWriteForm();
    ...
    function fn_deleteOldFile(fileDtlId) {
        AppAlert.confirm(
            '파일 삭제',
            '수정 완료 시 해당 파일이 영구 삭제됩니다. 삭제하시겠습니까?',
            '삭제',
            '취소',
            'delete_forever',
            'danger'
