...
<jsp:include page="mailbox.jsp"></jsp:include>
    ...
    <div class="flex-grow-1 p-4" style="background: #f4f7fe; overflow-y: auto;">
<!-- 리스트 -->
<div id="mail-list-view">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div style="color: #2c3e50; display: flex; align-items: center; gap: 10px;">
            ...
            <div style="display: flex; align-items: baseline; gap: 8px;" id="emailTitle">
        ...
        <%--<h4 class="fw-bold mb-0">전체 메일함</h4>--%>
        <div class="d-flex gap-2">
            <div class="input-group input-group-sm" style="width: 250px;">
                <select class="form-select" name="mode" style="max-width: 80px; padding-right: 20px; background-position: right 5px center; background-size: 10px;">
                    ...
                    <option value="emlSndrId">보낸사람</option>
                </select>
                <input type="text" class="form-control"  name="keyword" placeholder="메일 검색...">
                <button class="btn btn-outline-primary d-inline-flex align-items-center" style="border-color: var(--point-color); color: var(--point-color);" id="btnSearch"><span class="material-icons fs-6" id="brnSearchIconforStyle">search</span></button>
            </div>
            <a class="btn btn-primary btn-sm d-flex align-items-center" style="background-color:var(--point-color); border-color: var(--point-color);" href="/email/write">
    ...
    <div class="dashboard-card mb-3">
        <div class="p-2 d-flex align-items-center justify-content-between border-bottom">
            ...
            <div class="dropdown">
                <button class="btn btn-sm btn-light border text-secondary d-inline-flex align-items-center dropdown-toggle" data-bs-toggle="dropdown" onclick="toMailBoxFun()">
                        <span class="material-icons fs-6 v-align-middle">forward_to_inbox</span>이동
                </button>
...
<div id="mail-detail-view" style="display: none;">
    ...
    <div class="dashboard-card p-4">
        ...
        <div id="detail-content" class="py-4" style="min-height: 200px; max-height: 600px; overflow: auto; line-height: 1.6; white-space: pre-wrap;"></div>
...
<script>
    ...
    //쿠키 추가하기 2 => 메일박스
    function setCookieNumMailBox(num, mailboxttl) {
        console.log("setCookieNumMailBox:",num)
        const emailTitle = document.getElementById("emailTitle");
        emailTitle.innerHTML= `<span style="font-size: x-large; font-weight: 800;">메일</span>
                <span style="font-weight: normal; color: #717171; font-size: 15px;">| 받은 메일함</span>`
        emailTitle.innerHTML+= `<span style="font-weight: bold; color: #717171; font-size: 15px;">| `+mailboxttl+`</span>`
        const cookieName = "emlBoxNo";
        const cookieValue = num;
        // 쿠키 저장 (path=/ 를 설정해야 사이트 전역에서 접근 가능합니다)
        document.cookie = "pageFilter" + "=" + "mailBox" + ";path=/";
        document.cookie = cookieName + "=" + cookieValue + ";path=/";
        //const currentPagingItem = document.getElementById('currentPagingItem');
        //alert("쿠키가 저장되었습니다!");
        //if (currentPagingItem) {
    ...
    //메일박스에 넣기
    const handleMailBoxAction = (emlBoxNo, emlBoxTtl) => {
        // 1. 체크된 체크박스들만 가져오기
        ...
        if (selectedEmlNos.length === 0) {
            AppAlert.error('선택된 메일이 없습니다.', '', null, 'unsubscribe').then(res => {
            });
        ...
        if(emlBoxNo==0){
            AppAlert.confirm('메일박스 이동', '선택된 메일들을 정말<br> 메일함에서 제외하시겠습니까?', '확인', '취소', 'help_outline', 'primary')
                .then((result) => {
                    if (result.isConfirmed) {
                        axios.post('/email/readOrDelOrImpt', {
                            emlBoxNo: emlBoxNo,
                            readOrDelOrImpt: "updateMailbox",
                            emlNos: selectedEmlNos
                        }).then(res => {
                            // 성공 알림 (아이콘: check_circle)
                            AppAlert.success("메일 이동 완료", '', null, 'check_circle').then(result => {
                                location.href = "/email";
                            }); // (알람 리팩토링)

                        }).catch(e => {
                            // 에러 알림 (아이콘: error_outline)
                            AppAlert.error("오류 발생", e.message || e, null, 'error_outline').then(result => {
                                location.href = "/email";
                            }); // (알람 리팩토링)
                        });
                    } // if 끝
            ...
            AppAlert.confirm('메일박스 이동<br>"' + emlBoxTtl + '"', '선택된 메일들을 정말<br>"' + emlBoxTtl + '"에<br> 보관하시겠습니까?', '확인', '취소', 'help_outline', 'primary')
                .then((result) => {
                    if (result.isConfirmed) {
                        axios.post('/email/readOrDelOrImpt', {
                            emlBoxNo: emlBoxNo,
...
// 최종 출력: 불필요한 + 기호를 모두 제거하고 조합
            ...
            document.getElementById("detail-btns-footer").innerHTML=
                '<button class="btn btn-success btn-sm"  style="background-color:var(--point-color); border-color: var(--point-color);"onclick="emailReply('+mail.emlRcvrId+')">답장</button>' +
                '&nbsp;&nbsp;<button class="btn btn-success btn-sm" style="background-color:var(--point-color); border-color: var(--point-color);" onclick="emailForward('+mail.emlRcvrId+')">전달</button>'+
...
//답장//
    function emailReply(emlRcvrId){
// 1. 동적으로 form 생성
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '/email/reply'; // 컨트롤러의 @PostMapping 주소
...
// 1. 동적으로 form 생성
        const form = document.createElement('form');
        form.method = 'POST';
        form.action = '/email/forward'; // 컨트롤러의 @PostMapping 주소
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.post("email/updateIsImpt", data, {
                    ...
                    Swal.fire({
                        title: "삭제 완료",
                        text: "삭제된 메일은 '휴지통'에서 확인할 수 있습니다.",
                        icon: "success",
                        confirmButtonText: '목록으로',
                        /*alert("삭제된 메일은 '휴지통'에서 확인할 수 있습니다.");*/
                    }).then(result =>{location.href="/email";});
...
/////////전역함수 /////////////////////////////////////////////////////////////////////////////////////////
    ...
    const handleMailAction = (readOrDelOrImpt) => {
        ...
        if (readOrDelOrImpt == "isDeleted") {
            title = "삭제";
        } else if (readOrDelOrImpt == "isRead") {
            title = "읽음처리";
        } else if (readOrDelOrImpt == "isImpt") {
            title = "중요설정";
        }
         console.log("readOrDelOrImpt는",readOrDelOrImpt,"이고요 title은", title,"입니다.");
        AppAlert.confirm(title+' 확인', "정말 " + title + " 하시겠습니까?", '확인', '취소', 'help_outline', 'primary')
            .then((result) => {
                if (result.isConfirmed) {
                    axios.post('/email/readOrDelOrImpt', {
                        ...
                        // 성공 알림 (아이콘: check_circle)
                        AppAlert.success(title + " 완료", '', null, 'check_circle').then(result => {
                            location.href = "/email";
                        }); // (알람 리팩토링)
