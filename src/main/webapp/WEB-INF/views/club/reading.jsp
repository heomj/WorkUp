...
<style>
    ...
    /* 📖 진짜 두꺼운 책 전체 래퍼 */
    .thick-book-wrapper {
        /* 🚨 1000px -> 1500px: 원근감을 멀리 둬서, 글씨가 사다리꼴로 심하게 찌그러지는 현상 방지! */
        perspective: 1500px;
        width: 100%;
        max-width: 480px;
        height: 320px;
    ...
    .thick-book {
        ...
        /* 🚨 30deg -> 12deg: 위에서 꽂아보던 시선을, 살짝만 비스듬히 내려다보는 아주 편안한 각도로 수정! */
    ...
    .hardcover-back {
        position: absolute;
        top: -4px;    /* 위로 10px */
        bottom: -10px; /* 아래로 10px */
        left: -8px;    /* 왼쪽으로 8px */
        right: -8px;   /* 오른쪽으로 8px */

        background: linear-gradient(135deg, #df9600 0%, #834200 100%);
    ...
    /* 왼쪽 바닥 페이지 */
    .left-stack {
        left: 0; border-radius: 6px 0 0 6px; border-left: 2px solid #ddd;
        /* 🚨 사선 그림자를 버리고, 아래로 일직선으로 떨어지는 종이 두께감 적용 */
        box-shadow: inset -15px 0 20px rgba(0,0,0,0.05),
        0 1px 0 #ccc, 0 2px 0 #fff, 0 3px 0 #ccc, 0 4px 0 #fff,
        0 5px 0 #ccc, 0 6px 0 #fff, 0 7px 0 #bbb;
    }

    /* 오른쪽 바닥 페이지 */
    .right-stack {
        right: 0; border-radius: 0 6px 6px 0; border-right: 2px solid #ddd;
        /* 🚨 마찬가지로 아래로 떨어지는 수평 종이 두께감 */
        box-shadow: inset 15px 0 20px rgba(0,0,0,0.05),
        0 1px 0 #ccc, 0 2px 0 #fff, 0 3px 0 #ccc, 0 4px 0 #fff,
        0 5px 0 #ccc, 0 6px 0 #fff, 0 7px 0 #bbb;
    }
    ...
    .flipping-page.turn { transform: rotateY(-180deg); }
    .flipping-page.no-anim { transition: none !important; }
    ...
    .flip-front {
        border-radius: 0 6px 6px 0; border-right: 1px solid #ddd;
        box-shadow: inset 15px 0 20px rgba(0,0,0,0.05);
    }

    .flip-back {
        transform: rotateY(180deg);
        border-radius: 6px 0 0 6px; border-left: 1px solid #ddd;
        background: #fcfcfc;
        box-shadow: inset -15px 0 20px rgba(0,0,0,0.05);
    }
    /*  책모양 css 구역  */
    ...
    /* 소개 탭 UI 요소 */
    .intro-box {
        background-color: #f8f9fc; border-left: 4px solid var(--club-primary-color);
    ...
    /* ==========================================
       공지사항 테이블 (전자결재 스타일 이식)
       ========================================== */
    /* 1. 표 바탕 및 모든 셀 완벽한 흰색 강제 코팅! */
    #noticeTb,
    #noticeTb tbody tr,
    #noticeTb tbody td {
    ...
    /* 2. 마우스 호버 시 은은한 회색으로 변경! */
    #noticeTb tbody tr:hover td {
        background-color: #f8f9ff !important; /* 동호회 테마에 맞춰 살짝 쿨톤 회색으로 뺐습니다 */
    }

    /* ==========================================
       페이징 블럭 디자인 통일 (공지사항 & 포토갤러리 공용)
       ========================================== */
    #tdPagingArea,
    #galleryPagingArea {
        text-align: center;
    ...
    #galleryPagingArea .pagination {
        justify-content: center !important;
        margin-bottom: 0;
    }

    #tdPagingArea .pagination .page-link,
    #galleryPagingArea .pagination .page-link {
        border: none;
        margin: 0 3px;
    ...
    #galleryPagingArea .pagination .page-link:hover {
        background-color: #e2e6ea;
    }

    #tdPagingArea .pagination .page-item.active .page-link,
    #galleryPagingArea .pagination .page-item.active .page-link {
        background-color: var(--club-primary-color) !important; /* 포인트 컬러: 보라색! */
        color: #fff !important;
    }

    /* 공지사항 목록 제목(링크) 스타일 */
    #notice-tbody a {
        text-decoration: none; /* 밑줄 제거 마법! */
        color: #435971; /* 촌스러운 파란색 대신 차분하고 세련된 남색으로 변경 */
        transition: color 0.2s ease; /* 색이 변할 때 부드럽게 스르륵 변하도록 애니메이션 추가 */
    }

    /* 마우스를 제목 위에 올렸을 때 (Hover) */
    #notice-tbody a:hover {
        color: var(--club-primary-color); /* 동호회 메인 컬러(보라색)로 포인트! */
        text-decoration: underline; /* 마우스 올렸을 때만 밑줄이 살짝 보이게 (선택 사항) */
    }
        ...
        <div>
            <div style="color: #2c3e50; display: flex; align-items: center; gap: 10px;">
                ...
                <div style="display: flex; align-items: baseline; gap: 8px;">
                    ...
                    <span style="font-weight: normal; color: #717171; font-size: 15px;">| 페이지(Page)</span>
                </div>
            </div>

            <div style="font-size: 15px; color: #717171; margin-top: 8px; letter-spacing: -0.5px; font-weight: 400;">
                사내 독서 동호회 페이지(Page) 입니다. 가입 후 많은 활동 바랍니다.
            </div>
        </div>
        ...
        <div class="tab-content bg-white p-4 shadow-sm club-tab-content" id="clubTabContent">
            ...
            <div class="tab-pane fade show active" id="intro" role="tabpanel" aria-labelledby="intro-tab">
                ...
                <div class="row h-100">
                    ...
                    <!-- 책장 구역 -->
                    <div class="col-md-5 text-center mb-4 mb-md-0 d-flex flex-column pt-4">

                        <div class="position-relative d-flex align-items-center justify-content-center w-100">

                            <button class="btn btn-light shadow rounded-circle position-absolute z-3 d-flex justify-content-center align-items-center"
                                    style="width: 45px; height: 45px; left: -10px; border: 1px solid #ddd;"
                                    onclick="turnPrevPage()">
                                <span class="material-icons text-secondary">chevron_left</span>
                            </button>

                            <div class="thick-book-wrapper mx-3">
                                <div class="thick-book">
                                    <div class="hardcover-back"></div>
                                    <div class="page-stack left-stack" id="left-base-content"></div>
                                    <div class="page-stack right-stack" id="right-base-content"></div>
                                    <div class="flipping-page" id="flipper">
                                        <div class="flip-front" id="flipper-front"></div>
                                        <div class="flip-back" id="flipper-back"></div>
                                    </div>
                                </div>
                            </div>

                            <button class="btn btn-light shadow rounded-circle position-absolute z-3 d-flex justify-content-center align-items-center"
                                    style="width: 45px; height: 45px; right: -10px; border: 1px solid #ddd;"
                        ...
                        <div class="mt-4">
                            <span class="badge bg-light text-muted border px-3 py-2 rounded-pill shadow-sm">좌우 버튼으로 책장을 넘겨보세요</span>
                        </div>


                        <div class="d-flex justify-content-center align-items-center mt-4 text-muted" style="font-size: 13px; opacity: 0.8;">
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-tabler icon-tabler-database text-primary me-2" width="18" height="18" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
                            ...
                            <span>
                                실시간 도서 데이터 제공:
                                <a href="https://www.data4library.kr/" target="_blank" class="fw-bold text-decoration-none text-primary" style="letter-spacing: -0.5px;">
                                    도서관 정보나루 (국립중앙도서관)
                                </a>
                            </span>
                        </div>
                    ...
                    <div class="col-md-7 d-flex flex-column ps-md-4 pe-md-3 pt-3 h-100">
                        ...
                        <div class="mb-2">
                            <h4 class="fw-bolder mb-1" style="color: #435971; letter-spacing: -0.5px;">
                                사내 독서 동호회 <span style="color: var(--club-primary-color);">"페이지"</span>
                            </h4>
                        </div>
                        ...
                        <div class="executive-box p-4 rounded-4 mb-3 shadow-sm">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-3 pb-3 border-bottom">
                                <div>
                                    ...
                                    <p class="text-muted mb-0" style="font-size: 0.8rem;">회장: 000 팀장 | 총무: 000동 대리</p>
                                </div>
                            </div>
                    ...
                    <div class="bg-white p-4 rounded shadow-sm" style="border: 1px solid #e4e6ef;">
                        <form id="galleryWriteForm" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="gallery-title" name="clubBbsTtl" placeholder="예: 이번달 추천 도서 후기" required>
                            </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header text-white" style="background-color: var(--club-primary-color);">
                    <h5 class="modal-title d-flex align-items-center text-white" id="joinModalLabel">
                        <span class="material-icons me-2">group_add</span> 독서 동호회 페이지 가입 신청
                    </h5>
                ...
                <div class="modal-body p-4">
                    <form id="joinForm">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="joinGreeting" rows="4" placeholder="가장 좋아하는 책이나 가입하게 된 계기 등 간단한 인사를 남겨주세요!(최대 150자) "maxlength="150" required></textarea>
                        </div>
                    </form>
                </div>
...
<script type="text/javascript">
    ...
    function submitJoinRequest() {
        ...
        axios.post("/club/join", data, {
            ...
            .then(response => {
                // (알람 리팩토링) 가입 성공 알림창
                AppAlert.success('환영합니다!', '가입이 완료되었습니다. <br />책내음 가득한 활동을 시작해보세요!')
                    .then(() => {
    ...
    function leaveClub() {
        // (알람 리팩토링) 탈퇴 컨펌창으로 교체
        AppAlert.confirm('정말로 탈퇴하시겠습니까?', "독서 동호회 '페이지'를 떠나시게 됩니다.", '탈퇴하기', '머무르기', 'warning_amber', 'danger')
            .then((result) => {
                    ...
                    axios.post("/club/leave", data, {
                        ...
                        .then(response => {
                            if(response.data === "SUCCESS") {
                                // (알람 리팩토링) 탈퇴 성공 알림
                                AppAlert.success('탈퇴 완료', '언젠가 다시 책 앞에서 뵙기를 바랍니다.')
                                    .then(() => {
                                        location.reload();
                                    });
                            } else {
    ...
    //데이터 맵//////////////
    let bookContents = [
        { type: 'loading', title: 'Loading...', data: '실시간 도서 데이터를<br>불러오는 중입니다' },
        { type: 'loading', title: 'Loading...', data: '잠시만 기다려주세요' }
    ];

    let currIdx = 0; // 왼쪽 페이지 기준 인덱스
    let isFlipping = false; // 연타 방지 플래그
    let autoFlipTimer = null; // 자동 넘김 타이머

    // 인덱스가 음수일 때도 안전하게 순환시키는 헬퍼 함수
    function getIdx(i) {
        if(bookContents.length === 0) return 0;
        return (i % bookContents.length + bookContents.length) % bookContents.length;
    }

    function renderBookHtml(item) {
        if(item.type === 'title') {
            return `<span class="material-icons mb-2" style="font-size:45px; color:var(--club-primary-color);">menu_book</span>
                <h4 class="fw-bolder" style="color:#2c3e50;">\${item.title}</h4>
                <div class="small fw-bold text-muted mt-2 px-2" style="word-break:keep-all;">\${item.data}</div>`;
        } else if(item.type === 'cloud') {
            let tags = item.data.map(k => `<span style="color:var(--club-primary-color); font-weight:900; font-size:15px; margin:4px;">\${k}</span>`).join('');
            return `<div class="mb-3 text-muted fw-bold" style="font-size:12px;"># \${item.title}</div>
                <div class="d-flex flex-wrap justify-content-center">\${tags}</div>`;
        } else if (item.type === 'book') {
            return `<div class="mb-2 text-muted fw-bold" style="font-size:12px;"># \${item.title}</div>
                <img src="\${item.data.img}" style="width:80px; height:115px; object-fit:cover; box-shadow:3px 3px 10px rgba(0,0,0,0.2); margin-bottom:8px; border-radius:3px;">
                <div class="fw-bold text-dark" style="font-size:12px;">\${item.data.ttl}</div>
                <div class="text-muted" style="font-size:10px;">\${item.data.author}</div>`;
        } else if (item.type === 'loading') {
            // ✨ 로딩 전용 화면 UI: 스피너 아이콘 빙글빙글!
            return `<div class="spinner-border text-primary mb-3" role="status" style="width: 2rem; height: 2rem; color: var(--club-primary-color) !important;">
                      <span class="visually-hidden">Loading...</span>
                    </div>
                    <div class="small fw-bold text-muted px-2" style="word-break:keep-all;">\${item.data}</div>`;
        } else {
            return `<span class="material-icons text-warning mb-2" style="font-size:35px;">format_quote</span>
                <div style="font-size:14px; line-height:1.6; word-break:keep-all;" class="fw-bold text-dark px-2">"\${item.data}"</div>`;
        }
    }

    // 📚 휴식 상태(넘기지 않을 때)의 데이터 세팅
    function setDefaultState() {
        if(bookContents.length === 0) return;

        const flipper = document.getElementById('flipper');
        flipper.classList.add('no-anim');
        flipper.style.transform = 'rotateY(0deg)';

        document.getElementById('left-base-content').innerHTML = renderBookHtml(bookContents[getIdx(currIdx)]);
        document.getElementById('right-base-content').innerHTML = renderBookHtml(bookContents[getIdx(currIdx + 1)]);

        document.getElementById('flipper-front').innerHTML = renderBookHtml(bookContents[getIdx(currIdx + 1)]);
        document.getElementById('flipper-back').innerHTML = renderBookHtml(bookContents[getIdx(currIdx + 1)]);
    }

    // ▶️ 다음 페이지로 넘기기 (오른쪽 -> 왼쪽)
    function turnNextPage() {
        if(isFlipping) return;
        isFlipping = true;
        resetAutoFlipTimer(); // 수동 조작 시 자동 넘김 시간 초기화

        const flipper = document.getElementById('flipper');
        // 1. 넘길 준비 (플리퍼 뒷면에 다음 데이터 바르기)
        flipper.classList.add('no-anim');
        flipper.style.transform = 'rotateY(0deg)';
        document.getElementById('flipper-back').innerHTML = renderBookHtml(bookContents[getIdx(currIdx + 1)]);
        document.getElementById('right-base-content').innerHTML = renderBookHtml(bookContents[getIdx(currIdx + 2)]);
        // 강제 리플로우 (CSS 적용을 위해 필수)
        void flipper.offsetWidth;

        // 2. 휙! 넘기기
        flipper.classList.remove('no-anim');
        flipper.style.transform = 'rotateY(-180deg)';
        // 3. 완전히 넘어가면 인덱스 갱신 및 제자리 세팅
        setTimeout(() => {
            currIdx = getIdx(currIdx + 1);
            setDefaultState();
            isFlipping = false;
        }, 1200);
    }

    // ◀️ 이전 페이지로 넘기기 (왼쪽 -> 오른쪽 역재생 마법!)
    function turnPrevPage() {
        ...
        // 1. 역재생 준비 (플리퍼를 몰래 왼쪽으로 눕혀놓기)
        flipper.classList.add('no-anim');
        flipper.style.transform = 'rotateY(-180deg)';

        // 왼쪽에서 올라올 거니까, 플리퍼 뒷면(현재 눈에 보임)은 현재 왼쪽 데이터로 세팅
        document.getElementById('flipper-back').innerHTML = renderBookHtml(bookContents[getIdx(currIdx)]);
        // 플리퍼 앞면(오른쪽에 덮일 데이터)은 이전 오른쪽 데이터로 세팅
        document.getElementById('flipper-front').innerHTML = renderBookHtml(bookContents[getIdx(currIdx - 1)]);
        // 바닥에 깔릴 왼쪽 베이스는 이전 왼쪽 데이터로 세팅
        document.getElementById('left-base-content').innerHTML = renderBookHtml(bookContents[getIdx(currIdx - 1)]);
        void flipper.offsetWidth;
    ...
    function startAutoFlip() {
        autoFlipTimer = setInterval(turnNextPage, 6500); // 6.5초마다 다음 페이지
    }
    function resetAutoFlipTimer() {
        clearInterval(autoFlipTimer);
        startAutoFlip(); // 버튼 누르면 다시 6초 카운트다운 시작
    }


    // 🚀 3. Axios 연동 부분 수정 (데이터 받으면 교체!)
    function loadDynamicBookContents() {
        // 로딩 화면이 먼저 보이게 세팅 (데이터를 가져오기 전 상태)
        setDefaultState();
        axios.get('/club/books')
            .then(response => {
                // 데이터가 성공적으로 들어오면 진짜 데이터로 덮어씌움!
                bookContents = response.data;
                currIdx = 0; // 인덱스를 다시 0으로 초기화

                // 새로운 데이터로 화면 새로고침 후 자동 넘김 시작!
                setDefaultState();
                startAutoFlip();
            })
            .catch(error => {
                console.error("실시간 도서 데이터를 불러오지 못했습니다: ", error);

                // 실패 시 비상용 화면으로 덮어씌움
                bookContents = [
                    { type: 'title', title: 'Page', data: "사내 독서 동호회<br>오프라인 모드" },
                    { type: 'quote', title: '서버 점검 중', data: "현재 실시간 데이터를<br>불러올 수 없습니다. 😥" }
                ];
                currIdx = 0;
                setDefaultState();
            });
    ...
    document.addEventListener("DOMContentLoaded", function() {
        loadClubMembers();
        loadDynamicBookContents(); // ✨ API 호출!
    });
    ...
    function submitNotice() {
        ...
        const data = { clubNo: 2, clubNtcTtl: title, clubNtcCn: content };
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    axios.post("/club/noticeDelete", data, {
                        ...
                        .then(response => {
                            // (알람 리팩토링) 자동 닫힘 성공 알림
                            AppAlert.autoClose('삭제 완료', '공지사항이 삭제되었습니다.', 'check_circle', 'success', 1500)
                                .then(() => {
                                    goToList();
                                });
                        })
                    ...
                    axios.post("/club/galleryWrite", formData, {
                        ...
                        .then(response => {
                            if(response.data === "SUCCESS" || response.status === 200) {
                                // (알람 리팩토링) 성공 후 자동 닫힘 알림
                                AppAlert.autoClose('등록 완료!', '사진이 성공적으로 등록되었습니다!', 'check_circle', 'success', 1500)
                                    .then(() => {
                                        document.getElementById('galleryWriteForm').reset();
                                        document.getElementById('image-preview-container').innerHTML = '<span class="text-muted small align-self-center w-100 text-center">선택된 이미지가 없습니다.</span>';
                                        document.getElementById('gallery-write-area').style.display = 'none';
            ...
            .then((result) => {
                if (result.isConfirmed) {
                    axios.post("/club/galleryDelete", clubBbsNo, {
                        ...
                        .then(response => {
                            // (알람 리팩토링) 자동 닫힘 완료 알림
                            AppAlert.autoClose('삭제 완료', '사진이 삭제되었습니다.', 'check_circle', 'success', 1500)
                                .then(() => {
                                    goToGalleryList();
                                });
                        })
