        ...
        <div>
            <div style="color: #2c3e50; display: flex; align-items: center; gap: 10px;">
                ...
                <div style="display: flex; align-items: baseline; gap: 8px;">
                    ...
                    <span style="font-weight: normal; color: #717171; font-size: 15px;">| 아스트로(Astro)</span>
                </div>
            </div>

            <div style="font-size: 15px; color: #717171; margin-top: 8px; letter-spacing: -0.5px; font-weight: 400;">
                사내 천문 동호회 아스트로(Astro) 입니다. 가입 후 많은 활동 바랍니다.
            </div>
        </div>
        ...
        <div class="tab-content bg-white p-4 shadow-sm club-tab-content" id="clubTabContent">
            ...
            <div class="tab-pane fade show active" id="intro" role="tabpanel" aria-labelledby="intro-tab">
                <div class="row h-100">
                    ...
                    <div class="col-md-5 text-center mb-4 mb-md-0 d-flex flex-column pt-3">
                        <div class="map-container" style="box-shadow: 0 10px 30px rgba(105, 108, 255, 0.2); border: 5px solid #fff;">
                            <div id="star-loading" class="loading-stars">우주를 그리는 중... 🚀</div>
                            <div id="star-map"></div>
                        </div>
                        <div class="mt-3">
                            <span class="badge bg-light text-muted border px-3 py-2 rounded-pill shadow-sm">실시간 하늘 (기준: 대전광역시)</span>
                        </div>
                        ...
                        <div class="d-flex justify-content-center align-items-center mt-4 text-muted" style="font-size: 13px; opacity: 0.8;">
                            ...
                            <span>
                                실시간 성도 데이터 제공:
                                <a href="https://github.com/ofrohn/d3-celestial" target="_blank" class="fw-bold text-decoration-none text-primary" style="letter-spacing: -0.5px;">
                                    d3-celestial (오픈소스 천문 데이터)
                                </a>
                            </span>
                        </div>
                    ...
                    <div class="col-md-7 d-flex flex-column ps-md-4 pe-md-3 pt-3 h-100">
                        ...
                        <div class="mb-2">
                            <h4 class="fw-bolder mb-1" style="color: #435971; letter-spacing: -0.5px;">
                                사내 천문 동호회 <span style="color: var(--club-primary-color);">"아스트로"</span>
                            </h4>
                        </div>
                        ...
                        <div class="executive-box p-4 rounded-4 mb-3 shadow-sm">
                            ...
                            <div class="d-flex align-items-center gap-3 mb-3 pb-3 border-bottom">
                                <div>
                                    ...
                                    <p class="text-muted mb-0" style="font-size: 0.8rem;">회장: 000 팀장 | 총무: 000 팀장</p>
                                </div>
                            </div>
            ...
            <div class="tab-pane fade" id="gallery" role="tabpanel" aria-labelledby="gallery-tab">
                ...
                <div id="gallery-write-area" style="display: none;">
                    <div class="d-flex justify-content-between align-items-center mb-3 pb-3 border-bottom">
                        ...
                        <!-- 자동완성 -->
                        <button type="button" class="btn d-flex align-items-center fw-bold px-3" onclick="fillDummyGalleryData()"
                    ...
                    <div class="bg-white p-4 rounded shadow-sm" style="border: 1px solid #e4e6ef;">
                        <form id="galleryWriteForm" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="text" class="form-control" id="gallery-title" name="clubBbsTtl" placeholder="예: 겨울철 오리온자리 관측" required>
                            </div>
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header text-white" style="background-color: var(--club-primary-color);">
                    <h5 class="modal-title d-flex align-items-center text-white" id="joinModalLabel">
                        <span class="material-icons me-2">group_add</span> 천문 동호회 아스트로 가입 신청
                    </h5>
                ...
                <div class="modal-body p-4">
                    <form id="joinForm">
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="joinGreeting" rows="4" placeholder="가장 좋아하는 별자리나 가입하게 된 계기 등 간단한 인사를 남겨주세요!(최대 150자)"
                ...
                <div class="modal-footer bg-light border-top-0 d-flex justify-content-between">
                    ...
                    <button type="button" class="btn d-flex align-items-center fw-bold px-3" onclick="fillDummyJoinData()"
                            style="background-color: #e2e6ea; color: #566a7f; border: 1px solid #d9dee3; border-radius: 6px; transition: 0.2s;"
                            onmouseover="this.style.backgroundColor='#d3d8de'" onmouseout="this.style.backgroundColor='#e2e6ea'">
                        <span class="material-icons me-1" style="font-size: 18px;">auto_fix_high</span> 자동완성
                    </button>
...
<script type="text/javascript">
    ...
    function submitJoinRequest() {
        ...
        axios.post("/club/join", data, {
            ...
            .then(response => {
                // (알람 리팩토링) 가입 성공 알림창
                AppAlert.success('환영합니다!', '가입이 완료되었습니다. <br/>별빛 가득한 활동을 시작해보세요!')
                    .then(() => {
    ...
    function leaveClub() {
        // (알람 리팩토링) 컨펌창으로 교체
        AppAlert.confirm('탈퇴하시겠습니까?', "천문 동호회 '아스트로'를 떠나시게 됩니다.", '탈퇴하기', '머무르기', 'warning_amber', 'danger')
            .then((result) => {
                    ...
                    axios.post("/club/leave", data, {
                        ...
                        .then(response => {
                            if(response.data === "SUCCESS") {
                                // (알람 리팩토링) 탈퇴 성공 알림
                                AppAlert.success('탈퇴 완료', '언젠가 다시 밤하늘 아래서 뵙기를 바랍니다.')
                                    .then(() => {
                                        location.reload();
                                    });
                            } else {
    ...
    // ==========================================
    // D3 실시간 성도(Star Map) 렌더링 로직 (거울 우주 탈출 완벽 패치!)
    // ==========================================
    function renderStarMap() {
        const width = 800;
        const height = 800;

        d3.select("#star-map").selectAll("*").remove();
        const svg = d3.select("#star-map")
            .append("svg")
            .attr("viewBox", `0 0 \${width} \${height}`);

        Promise.all([
            d3.json("https://cdn.jsdelivr.net/npm/d3-celestial/data/stars.6.json"),
            d3.json("https://cdn.jsdelivr.net/npm/d3-celestial/data/constellations.lines.json")
            ...
            // 대전의 머리 꼭대기(천정)를 중심으로 회전!
            projection.rotate([-lst, -lat, 0]);

            const path = d3.geoPath(projection);
            const graticule = d3.geoGraticule();

            // 🌟 핵심 마법: 우주 밖 시점 -> 땅에서 올려다보는 시점으로 변환! (X축 좌우 반전)
            const skyG = svg.append("g")
                .attr("transform", `translate(\${width}, 0) scale(-1, 1)`);

            // 이제 모든 별과 선은 반전된 skyG 그룹 안에 그립니다!
            skyG.append("path")
                .datum(graticule())
                .attr("d", path)
                .attr("fill", "none")
            ...
            const visibleStars = starsData.features.filter(d => d.properties.mag <= 4.5);
            const radiusScale = d3.scaleLinear().domain([-1, 6]).range([5, 0.5]);
            ...
            skyG.append("g")
                ...
                .enter().append("circle")
                .attr("class", "star")
                .attr("cx", d => { const p = projection(d.geometry.coordinates); return p ? p[0] : -9999; })
                .attr("cy", d => { const p = projection(d.geometry.coordinates); return p ? p[1] : -9999; })
                .attr("r", d => radiusScale(d.properties.mag))
                .attr("fill", "#ffffff");

            // 행성, 태양, 달 위치 계산 (이전과 동일)
            function getPlanetPos(planetName) {
                const orbits = {
                    "금성": { N: 76.67, i: 3.39, w: 54.89, a: 0.723, e: 0.006, M: 48.00 + 1.6021 * d, color: "#ffebb5", mag: -4.4 },
                    "화성": { N: 49.55, i: 1.84, w: 286.50, a: 1.523, e: 0.093, M: 18.60 + 0.5240 * d, color: "#ff884d", mag: 0.0 },
                    "목성": { N: 100.45, i: 1.30, w: 273.86, a: 5.202, e: 0.048, M: 19.89 + 0.0830 * d, color: "#e6d5b8", mag: -2.7 }
                };
                const orb = orbits[planetName];
                let M = orb.M % 360;
                let E = M + (180/Math.PI) * orb.e * Math.sin(M * Math.PI/180) * (1 + orb.e * Math.cos(M * Math.PI/180));
                let ra = (M + orb.w + orb.N) % 360;
                let dec = orb.i * Math.sin((M + orb.w) * Math.PI/180);
                return { name: planetName, coords: [ra, dec], color: orb.color, mag: orb.mag };
            }

            function getSunPos() {
                const M = (356.0470 + 0.9856002585 * d) % 360;
                const L = (280.460 + 0.9856474 * d) % 360;
                const rad = Math.PI / 180;
                const lon = L + 1.915 * Math.sin(M * rad) + 0.020 * Math.sin(2 * M * rad);
                const obl = 23.439 - 0.0000004 * d;
                const ra = Math.atan2(Math.cos(obl * rad) * Math.sin(lon * rad), Math.cos(lon * rad)) / rad;
                const dec = Math.asin(Math.sin(obl * rad) * Math.sin(lon * rad)) / rad;
                return { name: "태양", coords: [ra > 0 ? ra : ra + 360, dec], color: "#ffcc00", size: 14 };
            }
            ...
            function getMoonPos() {
                ...
                const obl = 23.439 - 0.0000004 * d;
                const ra = Math.atan2(Math.sin(lon * rad) * Math.cos(obl * rad) - Math.tan(lat * rad) * Math.sin(obl * rad), Math.cos(lon * rad)) / rad;
                const dec = Math.asin(Math.sin(lat * rad) * Math.cos(obl * rad) + Math.cos(lat * rad) * Math.sin(obl * rad) * Math.sin(lon * rad)) / rad;
                return { name: "달", coords: [ra > 0 ? ra : ra + 360, dec], color: "#f8f9fa", size: 10 };
            }

            const planetsData = ["금성", "화성", "목성"].map(name => getPlanetPos(name));
            planetsData.push(getSunPos());
            planetsData.push(getMoonPos());

            // 천체 도형 그리기 (반전된 skyG에 넣음)
            skyG.append("g")
                .selectAll("circle.planet")
                .data(planetsData)
                .enter().append("circle")
                .attr("class", "planet")
                .attr("cx", d => { const p = projection(d.coords); return p ? p[0] : -9999; })
                .attr("cy", d => { const p = projection(d.coords); return p ? p[1] : -9999; })
                .attr("r", d => d.size || radiusScale(d.mag) * 1.5)
                .attr("fill", d => d.color)
                .attr("stroke", "#ffffff")
                .attr("stroke-width", d => d.name === "태양" ? 0 : 0.5)
                .style("filter", d => d.name === "태양" ? "drop-shadow(0 0 8px rgba(255, 204, 0, 0.8))" : "none");

            // 💡 텍스트는 거꾸로 뒤집히면 안 되니까 원본 svg에 넣되, 좌표 X를 (width - p[0])로 수동 반전!
            svg.append("g")
                .selectAll("text.planet-label")
                .data(planetsData)
                .enter().append("text")
                .attr("class", "planet-label")
                .attr("x", d => { const p = projection(d.coords); return p ? (width - p[0]) + (d.size || 8) + 2 : -9999; })
                .attr("y", d => { const p = projection(d.coords); return p ? p[1] + 4 : -9999; })
                .text(d => d.name)
                .attr("fill", d => d.name === "태양" ? "#ffcc00" : "rgba(255,255,255,0.9)")
                .style("font-size", "12px")
                .style("font-weight", "900");

            // 🧭 동서남북 방위표 (하늘을 올려다보는 기준이라 왼쪽이 E, 오른쪽이 W가 맞습니다!)
            const compassRadius = 320;
            const cx = width / 2;
            const cy = height / 2;

            const directions = [
                { label: "N", x: cx, y: cy - compassRadius },
                { label: "S", x: cx, y: cy + compassRadius },
                { label: "E", x: cx - compassRadius, y: cy }, // 동 (왼쪽)
                { label: "W", x: cx + compassRadius, y: cy }  // 서 (오른쪽)
            ];

            svg.append("g")
                .selectAll("text.compass")
                .data(directions)
                .enter().append("text")
                .attr("class", "compass")
                .attr("x", d => d.x)
                ...
                .text(d => d.label)
                .attr("fill", "rgba(255, 204, 0, 0.8)")
                .style("font-size", "22px")
                .style("font-weight", "900")
                .style("letter-spacing", "2px");

        }).catch(err => {
            console.error("천문 데이터 로딩 에러:", err);
            document.getElementById('star-loading').innerText = "별자리를 불러오지 못했습니다";
        });
    }
    ...
    function submitNotice() {
        ...
        const data = { clubNo: 1, clubNtcTtl: title, clubNtcCn: content };
    ...
    function deleteGallery(clubBbsNo) {
        // (알람 리팩토링) 갤러리 삭제 컨펌창
        AppAlert.confirm('사진을 삭제하시겠습니까?', '정말로 이 아름다운 사진을 지우시겠습니까?', '삭제', '취소', 'delete', 'danger')
            .then((result) => {
                if (result.isConfirmed) {
    ...
    // 가입인사 자동완성 함수
    function fillDummyJoinData() {
        const joinGreeting = document.getElementById('joinGreeting');

        if(joinGreeting) {
            // 천문 동호회 감성에 맞춘 텍스트 🔭
            joinGreeting.value = "안녕하세요! \n평소 밤하늘의 별과 우주에 관심이 많은 신입입니다. \n맑은 날 동료들과 함께 천체 망원경으로 관측하며 힐링하고 싶어 가입을 신청합니다. 잘 부탁드립니다!";
        }
    ...
    //포토갤러리 자동완성 함수
    function fillDummyGalleryData() {
        const titleInput = document.getElementById('gallery-title');
        const contentInput = document.getElementById('gallery-content');

        if(titleInput && contentInput) {
            titleInput.value = "대전 천문대 겨울 밤하늘 관측 후기";
            contentInput.value = "지난 주말 천문대 관측 사진입니다!\n 직접 찍은 베스트 컷 몇 장 공유합니다.";
        }
