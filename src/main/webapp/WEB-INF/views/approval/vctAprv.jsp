...
<style>
    ...
    /* 페이지 전체의 스크롤바를 숨겨서 한 화면에 고정된 느낌 */
    html, body, .main-content, .page-wrapper {
        -ms-overflow-style: none !important; /* IE, Edge */
        scrollbar-width: none !important;    /* Firefox */
    }

    /* 2. 크롬, 사파리 등 웹킷 브라우저 전용 */
                ...
                <div class="card-body p-4 mx-auto bg-white shadow-sm document-container" id="pdfArea">
                    ...
                    <div class="mb-4 p-2 bg-light rounded border border-dashed text-center">
                        <select class="form-select form-select-sm border-primary-subtle d-inline-block w-75" id="prevRequestSelect" onchange="autoFillForm()">
                            <option value="">-- 휴가 신청 내역 불러오기 (선택 시 자동 완성) --</option>
                            <c:forEach var="VacationDocumentVO" items="${VacationDocumentVOList}">

                                <fmt:formatDate value="${VacationDocumentVO.vctDocBgng}" pattern="yyyy-MM-dd (E)" var="fmtBgng" />
                                <fmt:formatDate value="${VacationDocumentVO.vctDocEnd}" pattern="yyyy-MM-dd (E)" var="fmtEnd" />

                                <option value="${VacationDocumentVO.attTypeId}"
                                        data-bgng="${fmtBgng}"
                                        data-end="${fmtEnd}"
                                        data-res="${VacationDocumentVO.vctDocRsn}"
                                        data-type="${VacationDocumentVO.vctDocCd}"
                                        data-use="${VacationDocumentVO.vctTotalDays}"
                                    ${param.attTypeId == VacationDocumentVO.attTypeId ? 'selected' : ''}>
                                    [${VacationDocumentVO.vctDocCd}] ${fmtBgng} ~ ${fmtEnd}
                                </option>
                            </c:forEach>
                        </select>
                    </div>
                        ...
                        <table class="main-content-table">
                            ...
                            <tr>
                                ...
                                <td colspan="5" class="p-0">
                                    <input type="text" id="vctType" class="form-control border-0 text-center w-100 h-100 align-middle" placeholder="휴가 신청 내역을 선택하면 자동으로 입력됩니다." readonly style="background-color: #fff;">
                                </td>
                            </tr>

                                <th class="bg-label py-2">시작일시</th>
                                <td class="p-0">
                                    <div id="startDate" class="form-control border-0 text-center w-100 h-100 px-0 d-flex flex-column justify-content-center" style="background-color: #fff; line-height: 1.4; min-height: 55px; font-size: 14px; white-space: nowrap; letter-spacing: -0.5px;">
                                        <span class="text-muted small">YYYY-MM-DD</span>
                                    </div>
                                </td>

                                <th class="bg-label py-2">종료일시</th>
                                <td class="p-0">
                                    <div id="endDate" class="form-control border-0 text-center w-100 h-100 px-0 d-flex flex-column justify-content-center" style="background-color: #fff; line-height: 1.4; min-height: 55px; font-size: 14px; white-space: nowrap; letter-spacing: -0.5px;">
                                        <span class="text-muted small">YYYY-MM-DD</span>
                                    </div>
                                </td>
                                ...
                                <td class="p-0">
                                    <div id="vctUsedDays" class="form-control border-0 text-center w-100 h-100 d-flex flex-column justify-content-center" style="background-color: #fff; min-height: 55px;">
                                        -
                                    </div>
                                </td>
                            </tr>
                            ...
                            <tr>
                                ...
                                <td colspan="5" class="p-0 align-top">
                                    <textarea id="reason" class="form-control border-0 text-start w-100" placeholder="휴가 입력사유는 목록 선택 시 자동 입력 됩니다." readonly style="resize: none; background-color: #fff; padding: 15px; white-space: pre-wrap; height: 160px; line-height: 1.6;"></textarea>
                                </td>
                            </tr>
                        </table>
...
<script>
    ...
    function autoFillForm() {
        ...
        if (!selectedOption.value) {
            ...
            document.getElementById('vctType').value = "";
            document.getElementById('reason').value = "";
            // div 태그이므로 innerHTML로 텍스트를 초기화합니다.
            document.getElementById('startDate').innerHTML = "<span class='text-muted small'>YYYY-MM-DD</span>";
            document.getElementById('endDate').innerHTML = "<span class='text-muted small'>YYYY-MM-DD</span>";
            document.getElementById('vctUsedDays').innerHTML = "-";
            return;
        }
        ...
        // 계산된 일수를 화면에 출력
        document.getElementById('vctUsedDays').innerHTML = use + ' 일';
        console.log("선택된 휴가 신청 번호(attTypeId):", attTypeId);
    }
    //셀렉트 박스 끝
    ...
    document.getElementById('submitAprvBtn').addEventListener('click', function() {
        ...
        if(!attTypeId) {
            // 💡 리팩토링: 내역 선택 누락
            AppAlert.warning('입력 확인', '휴가 신청 내역을 먼저 선택해 주세요.', 'prevRequestSelect', 'content_paste');
            return;
        }
        ...
        // 💡 리팩토링: 상신 확인 컨펌창
        AppAlert.confirm('상신하시겠습니까?', '상신 후에는 수정이 불가능합니다.', '상신하기', '취소', 'send', 'primary')
            .then((result) => {
                if (result.isConfirmed) { // '상신하기' 버튼을 눌렀을 때만 아래 로직 실행
                    // 화면의 데이터 수집
                    const isUrgent = document.getElementById('urgentCheck').checked ? 'Y' : 'N'; //긴급
                    const isOpen = document.getElementById('openY').checked ? 'Y' : 'N';    //비공개

                    // 기안자를 제외한 진짜 결재자 배열만 추출
                    ...
                    formData.append("aprvSe", "APRV01002");
                    ...
                    axios.post("/approval/submit", formData, {
                        ...
                        .then(response => {
                            if(response.data === "SUCCESS") {
                                // 💡 리팩토링: 상신 완료 (자동 닫힘 후 이동)
                                AppAlert.autoClose('상신 완료!', '성공적으로 상신되었습니다.')
                                    .then(() => {
                                        window.location.href = "/approval/myAprvBoard"; // 결재 목록 페이지로 이동
                                    });
                            } else if (response.data === "FAIL_LEAVE") {
                                // 💡 리팩토링: 연차 부족 에러
                                AppAlert.error('상신 실패', '잔여 연차가 부족하거나 처리 중 오류가 발생했습니다.');
                            }
                        })
                        .catch(error => {
                            console.error("상신 에러:", error);
                            // 💡 리팩토링: 서버 통신 에러
                            AppAlert.error('오류 발생', '상신 처리 중 서버 오류가 발생했습니다.');
                        });
                }
            }); // end AppAlert.confirm
    });
    ...
    // ==========================================
    // 💡 Driver.js 휴가 신청 페이지 튜토리얼
    // ==========================================
    function startVctTutorial() {
        ...
        const driverObj = driver({
            ...
            steps: [
                {
                    element: '#prevRequestSelect', // 휴가 내역 불러오기 셀렉트 박스
                    popover: {
                        title: '휴가 내역 불러오기',
                        description: '가장 먼저 이곳을 클릭하여 <span style="color: #696cff; font-weight: bold;">근태에서 신청한<br /> 휴가 내역</span>을 선택해 주세요.<br><br>선택하면 아래 문서 내용이 자동으로 입력<br />됩니다.' +
                            '<div style="background-color: #fff2f0; padding: 10px; border-radius: 6px; margin-top: 12px; font-size: 0.85rem; color: #ff3e1d; border: 1px solid rgba(255, 62, 29, 0.2); line-height: 1.4;"><b>주의사항</b><br>새로운 휴가는 [근태] 메뉴에서 먼저 신청 후 불러올 수 있습니다. (직접 작성 불가)</div>',side: "bottom", align: 'center'
                    }
                },
                {
                ...
                {
                    element: '#workTagSelect', // 업무 태그 셀렉트 박스 타겟팅
                    popover: {
                        title: '업무 태그 선택',
