                ...
                <div class="card-body p-4 mx-auto bg-white shadow-sm document-container" id="pdfArea">
                    ...
                    <div class="mb-4 p-2 bg-light rounded border border-dashed text-center">
                        <select class="form-select form-select-sm border-primary-subtle d-inline-block w-75" id="prevRequestSelect" onchange="autoFillForm()">
                            <option value="">- 초과근무 신청 내역 불러오기 (선택 시 자동 완성) --</option>
                            <c:forEach var="OvertimeDocumentVO" items="${overtimeDocumentVOList}">

                                <fmt:formatDate value="${OvertimeDocumentVO.excsWorkDocBgng}" pattern="yyyy-MM-dd (E)" var="fmtBgng" />

                                <option value="${OvertimeDocumentVO.attTypeId}"
                                        data-bgng="${fmtBgng}"
                                        data-res="${OvertimeDocumentVO.excsWorkDocRes}"
                                    ${param.attTypeId == OvertimeDocumentVO.attTypeId ? 'selected' : ''}>
                                    [초과근무] ${fmtBgng}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div id="docBody">
                        ...
                        <table class="main-content-table">
                            <tr>
                                ...
                                <td id="deptNm" style="width: 18%; text-align: center;">${deptNm}</td>
                                <th class="bg-label" style="width: 15%;">직급</th>
                                <td id="posNm" style="width: 18%; text-align: center;">${empJbgd}</td>
                                <th class="bg-label" style="width: 15%;">성명</th>
                                <td id="writerNm" style="width: 19%; text-align: center;">${empNm}</td>
                            </tr>
                            <tr>
                                <th class="bg-label py-2">초과근무 일자</th>
                                <td colspan="5" class="p-0">
                                    <input type="text" id="startDate" class="form-control border-0 text-center w-100 h-100 align-middle" placeholder="목록 선택 시 자동으로 입력됩니다." readonly style="background-color: #fff; min-height: 45px;">
                                </td>
                            </tr>
                            <tr>
                                <th class="bg-label py-4">신청 사유<br>(상세히 기재)</th>
                                <td colspan="5" class="p-0 align-top">
                                    <textarea id="reason" class="form-control border-0 p-3 w-100" rows="8" placeholder="초과근무 사유 및 진행할 업무 내용을 상세히 입력하세요" style="resize: none;"></textarea>
                                </td>
                            </tr>
                        </table>
                        <div class="mt-4 fw-bold text-end pe-3">&nbsp;&nbsp;끝.</div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    document.getElementById('applyApprovalLineBtn').addEventListener('click', function() {
        ...
        for(let i = 0; i < rows.length; i++) {
            ...
            selectedApprovalLine.push({ aprvLnLvl: i, empId: empId, name: name, rank: rank, dept: dept });
        }
        for(let i = 2; i <= 5; i++) {
        ...
        // (알람 리팩토링) Swal.fire를 AppAlert.autoClose로 변경
    ...
    document.getElementById('submitAprvBtn').addEventListener('click', function() {
        ...
        // (알람 리팩토링) 유효성 검사 alert들을 AppAlert.warning으로 변경
        if(!aprvWorkTag) {
            AppAlert.warning('선택 확인', '업무 태그를 선택해 주세요.', 'workTagSelect', 'local_offer');
            return;
        }
        if(!attTypeId) {
            AppAlert.warning('입력 확인', '초과근무 신청 내역을 먼저 선택해 주세요.', 'prevRequestSelect', 'history');
            return;
        }
        if(!docTitle) {
        ...
        // (알람 리팩토링) 상신 확인 confirm을 AppAlert.confirm으로 변경
        AppAlert.confirm('상신하시겠습니까?', '상신 후에는 내용 수정이 불가능할 수 있습니다.', '상신하기', '취소', 'send', 'primary')
            .then((result) => {
                if (result.isConfirmed) {
                    ...
                    formData.append("aprvSe", "APRV01004");
                    ...
                    axios.post("/approval/submit", formData, {
                        ...
                        .then(response => {
                            if(response.data === "SUCCESS") {
                                // (알람 리팩토링) 성공 알림 후 페이지 이동
                                AppAlert.success('상신 완료!', '초과근무 신청이 성공적으로 상신되었습니다.')
                                    .then(() => {
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (selectBox.value) { autoFillForm(); }
    });
    ...
    function startBztripTutorial() {
        ...
        const driverObj = driver({
            ...
            steps: [
                { element: '#prevRequestSelect', popover: { title: '출장 내역 불러오기', description: '가장 먼저 이곳을 클릭하여 <span style="color: #696cff; font-weight: bold;">근태에서 신청한<br /> 출장 내역</span>을 선택해 주세요.', side: "bottom", align: 'center' } },
                { element: '.btn-aprv-line', popover: { title: '결재선 지정', description: '문서가 작성되면 이 버튼을 눌러 <span style="color: #696cff; font-weight: bold;">결재할 사람</span>을 지정해야 합니다.', side: "bottom", align: 'end' } },
                { element: '#docTitle', popover: { title: '문서 제목 및 설정', description: '우측 패널에서 <span style="color: #696cff; font-weight: bold;">문서의 제목</span>을 입력해 주세요.', side: "left", align: 'start' } },
                { element: '#submitAprvBtn', popover: { title: '결재 상신', description: '마지막으로 <span style="color: #28a745; font-weight: bold;">상신 버튼</span>을 눌러 결재를 올립니다.', side: "bottom", align: 'end' } },
                {   element: '#menuWrapper',
                    popover: {
                        title: 'AI 검토 (말 다듬)',
